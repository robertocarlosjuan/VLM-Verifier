...
class VIMAEnvBase(gym.Env):
    ...
    def save_screenshot(self, config, file_path='./screenshot_image.png', image_size=None):
        ...
        rgba_image = Image.fromarray(color)
        rgba_image.save(file_path)

    def save_new_ss(self, config, file_path='./new_screenshot_image.png', image_size=None):
        """Render RGB-D image from an angled perspective looking at the origin."""
        if not image_size:
            image_size = config["image_size"]

        # OpenGL camera settings.
        # Keep the original up vector calculation based on camera rotation
        updir_local = np.float32([0, -1, 0]).reshape(3, 1)
        rotation = p.getMatrixFromQuaternion(
            config["rotation"], physicsClientId=self.client_id
        )
        rotm = np.float32(rotation).reshape(3, 3)
        updir = (rotm @ updir_local).reshape(-1)

        # --- Change: Set a fixed target point (origin) instead of looking straight ahead ---
        lookat = np.array([0.0, 1.0, 0.0], dtype=np.float32) # Look at the world origin
        # --- End Change ---

        focal_len = config["intrinsics"][0]
        znear, zfar = config["zrange"]
        # Use the position from the config as the camera location
        camera_position = config["position"]
        viewm = p.computeViewMatrix(
            camera_position, lookat, updir, physicsClientId=self.client_id
        )
        fovh = (image_size[0] / 2) / focal_len
        fovh = 180 * np.arctan(fovh) * 2 / np.pi
